...
import logging
import json
from util.tools.weather import get_alerts
from util.tools.db_tool import get_daily_report_data
...
class Superintendent_Agent:
    ...
    def generate_daily_report(self):
        ...
        query = f"Build a final daily report from a construction site to give the superintendent in charge of the duty. Use the worker information: {daily_response}, suppose you found some discrepancies across reports. Take into consideration weather conditions {weather_response}"
        response = self.main_llm.invoke(query)
        return response

agent = Superintendent_Agent()

response = agent.generate_daily_report()
