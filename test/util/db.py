...
class DataBase:
    ...
    def _build_query(self,cols=['t.id','t.metadata.file_name'],year=None,type='',region='',customer='',product='') -> str:
        search = ','.join(cols)
        query = rf"""SELECT {search} FROM WORKER_DATA t WHERE json_query(metadata, '$.report_date.date()?(@ > "2000-01-01T00:00")') IS NOT NULL"""
        if year:
    ...
    def update_db_records(self):
        with self._get_connection() as connection:
            ...
            try:
                query = "INSERT INTO WORKER_DATA (file_name,metadata,content) VALUES(:1,:2,:3)"
        ...
        with self._get_connection() as connection:
            ...
            cursor.execute("""
                CREATE TABLE WORKER_DATA (
                    ...
                    worker_name varchar2(256),
                    content VARCHAR2(256),
                    PRIMARY KEY (id))""")

            connection.commit()
            logger.info('Table created with name: WORKER_DATA')
